(function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    async function sendToN8nWebhook(message, action) {
      ...
      // Ensure sessionId and chatInput are always included
      const sessionId = getSessionId();
      const chatInput = message || '';
      
      const payload = {
        action: action || 'sendMessage',
        [config.chatSessionKey]: sessionId, // Always include sessionId
        [config.chatInputKey]: chatInput,  // Always include chatInput (user query)
        ...config.metadata
      };
      ...
      try {
        ...
        console.error('Webhook URL:', config.webhookUrl);
        ...
        // Show more detailed error message in chat
